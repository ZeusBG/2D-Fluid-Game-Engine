...
namespace loguru
{
    ...
    bool add_file(const char* path_in, FileMode mode, Verbosity verbosity)
    {
        ...
        _snprintf(file_abs->path, sizeof(file_abs->path) - 1, "%s", path);
        _snprintf(file_abs->mode_str, sizeof(file_abs->mode_str) - 1, "%s", mode_str);
        stat(file_abs->path, &file_abs->st);
        file_abs->fp = file;
    ...
    static void on_callback_change()
    {
        ...
        for (const auto& callback : s_callbacks) {
            s_max_out_verbosity = std::max<int>(s_max_out_verbosity, callback.verbosity);
        }
    }
    ...
    void get_thread_name(char* buffer, unsigned long long length, bool right_align_hext_id)
    {
        ...
        auto thread = pthread_self();
            ...
            if (const char* name = get_thread_name_ptls()) {
                _snprintf(buffer, length, "%s", name);
            } else {
                buffer[0] = 0;
            }
        #else
        ...
        if (buffer[0] == 0) {
            ...
            if (right_align_hext_id) {
                _snprintf(buffer, length, "%*X", length - 1, static_cast<unsigned>(thread_id));
            } else {
                _snprintf(buffer, length, "%X", static_cast<unsigned>(thread_id));
            }
        }
#elif LOGURU_WINTHREADS
    ...
    std::string stacktrace_as_stdstring(int skip)
    {
        ...
        for (int i = num_frames - 1; i >= skip; --i) {
            ...
            if (dladdr(callstack[i], &info) && info.dli_sname) {
                ...
                _snprintf(buf, sizeof(buf), "%-3d %*p %s + %zd\n",
                ...
                _snprintf(buf, sizeof(buf), "%-3d %*p %s\n",
    ...
    static void print_preamble(char* out_buff, size_t out_buff_size, Verbosity verbosity, const char* file, unsigned line)
    {
        ...
        if (verbosity <= Verbosity_FATAL) {
            _snprintf(level_buff, sizeof(level_buff) - 1, "FATL");
        } else if (verbosity == Verbosity_ERROR) {
            _snprintf(level_buff, sizeof(level_buff) - 1, "ERR");
        } else if (verbosity == Verbosity_WARNING) {
            _snprintf(level_buff, sizeof(level_buff) - 1, "WARN");
        } else {
            _snprintf(level_buff, sizeof(level_buff) - 1, "% 4d", verbosity);
        }

        _snprintf(out_buff, out_buff_size, "%04d-%02d-%02d %02d:%02d:%02d.%03lld (%8.3fs) [%-*s]%*s:%-5u %4s| ",
            1900 + time_info.tm_year, 1 + time_info.tm_mon, time_info.tm_mday,
            time_info.tm_hour, time_info.tm_min, time_info.tm_sec, ms_since_epoch % 1000,
            uptime_sec,
